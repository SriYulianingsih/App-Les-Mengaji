...
<div class="container mx-auto p-6 max-w-5xl">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Lakukan pembaruan data pada profil santri
                <strong><?= esc($santri['nama']) ?></strong>.
            </p>
        </div>
        <a href="/admin/santri"
    ...
    <form action="/admin/santri/update/<?= $santri['id'] ?>" method="POST" enctype="multipart/form-data"
        class="space-y-8">
        <?= csrf_field() ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="pendidikan_terakhir"
                                value="<?= old('pendidikan_terakhir', $santri['pendidikan_terakhir']) ?>"
                                class="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all text-sm font-medium">
                        </div>
                    </div>
            ...
            <div class="space-y-8">
                <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                    ...
                    <div class="space-y-5">
                        <div class="space-y-2">
                            <label class="text-xs font-black text-slate-500 uppercase tracking-wider ml-1">Status
                                Keaktifan</label>
                            <select name="status"
                                class="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all text-sm font-bold <?= $santri['status'] == 'aktif' ? 'text-emerald-600' : 'text-rose-600' ?>">
                                <option value="aktif"
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="orangtua_id" required
                                ...
                                <option value="<?= $ortu['id'] ?>"
                                    <?= old('orangtua_id', $santri['orangtua_id']) == $ortu['id'] ? 'selected' : '' ?>>
                                    <?= esc($ortu['nama_ayah']) ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
                ...
                <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                    ...
                    <div class="space-y-4">
                        <div
                            class="relative group aspect-[3/4] rounded-2xl overflow-hidden border-4 border-slate-50 shadow-inner bg-slate-100">
                            <?php if (!empty($santri['foto'])) : ?>
                            <img id="preview-img" src="/uploads/santri/<?= $santri['foto'] ?>"
                            ...
                            <label for="file-upload"
                                ...
                                <div
                                    class="bg-white/20 backdrop-blur-md px-4 py-2 rounded-xl border border-white/30 text-white text-xs font-bold">
                                    GANTI FOTO
                                </div>
                            </label>
                        </div>
                        ...
                        <p class="text-[10px] text-center text-slate-400 font-medium">Klik pada gambar untuk mengunggah
                            foto baru</p>
                    </div>
                </div>
            </div>
        ...
        <div class="flex justify-end pt-4">
            <button type="submit"
                class="bg-indigo-600 text-white font-bold px-10 py-4 rounded-2xl shadow-xl shadow-indigo-100 hover:bg-slate-900 transition-all transform hover:-translate-y-1 active:scale-95">
