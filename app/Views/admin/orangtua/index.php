...
<div class="container mx-auto p-6 max-w-7xl">
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Kelola Data Orang Tua</h1>
            <p class="text-sm text-gray-500 mt-1">Data wali dan orang tua santri.</p>
        </div>
        <a href="/admin/orangtua/create"
            class="bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-medium px-5 py-2.5 rounded-xl text-sm flex items-center shadow-lg shadow-emerald-100 transition-all duration-200 transform hover:-translate-y-0.5">
        ...
        <form action="/admin/orangtua" method="get" class="flex flex-col sm:flex-row gap-3">
            <div class="relative w-full md:w-1/3">
                ...
                <input type="text" name="q" value="<?= esc($keyword) ?>"
                    placeholder="Cari nama ayah, ibu, atau no. hp..."
                    class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all text-sm">
            </div>
                    ...
                    <tr class="hover:bg-emerald-50/30 transition-all duration-150">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-xl bg-emerald-50 border border-emerald-100 flex items-center justify-center text-emerald-600 font-bold text-sm shadow-sm flex-shrink-0">
                                    <?= strtoupper(substr((string)esc($ortu['nama_ayah']), 0, 1)) ?>
                                </div>
                                <div>
                                    <div class="text-gray-900 font-semibold text-base">Ayah:
                                        <?= esc($ortu['nama_ayah']) ?></div>
                        ...
                        <td class="px-6 py-4">
                            <span
                                class="text-gray-700 font-medium bg-gray-50 px-2.5 py-1 rounded-md text-xs border border-gray-100 inline-flex items-center">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <span
                                class="px-2.5 py-1 text-xs font-bold rounded-full inline-block bg-emerald-50 text-emerald-700 border border-emerald-100">
                                Terhubung (<?= esc($ortu['username']) ?>)
                            </span>
                            <?php else : ?>
                            <span
                                class="px-2.5 py-1 text-xs font-bold rounded-full inline-block bg-gray-50 text-gray-500 border border-gray-100">
                                Belum Ada Akun
                            </span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="/admin/orangtua/detail/<?= $ortu['id'] ?>"
                                ...
                                <a href="/admin/orangtua/edit/<?= $ortu['id'] ?>"
