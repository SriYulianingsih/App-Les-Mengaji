...
<div class="container mx-auto p-6 max-w-7xl" x-data="{ activeTab: 'admin' }">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div
            class="bg-white p-4 rounded-xl border border-slate-100 flex items-center gap-4 hover:shadow-sm transition-all">
            <div class="w-12 h-12 rounded-lg bg-slate-100 text-slate-700 flex items-center justify-center text-xl"><i
                    class="fas fa-user-shield"></i></div>
            <div>
                <p class="text-xs text-slate-500 font-medium">Total Admin</p>
                <p class="text-xl font-bold text-slate-800"><?= count($adminUsers) ?> <span
                        class="text-sm font-normal text-slate-400">Akun</span></p>
            </div>
        </div>
        <div
            class="bg-white p-4 rounded-xl border border-slate-100 flex items-center gap-4 hover:shadow-sm transition-all">
            <div class="w-12 h-12 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center text-xl"><i
                    class="fas fa-chalkboard-teacher"></i></div>
            <div>
                <p class="text-xs text-slate-500 font-medium">Total Akun Guru</p>
                <p class="text-xl font-bold text-slate-800"><?= count($guruUsers) ?> <span
                        class="text-sm font-normal text-slate-400">Akun</span></p>
            </div>
        </div>
        <div
            class="bg-white p-4 rounded-xl border border-slate-100 flex items-center gap-4 hover:shadow-sm transition-all">
            <div class="w-12 h-12 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center text-xl"><i
                    class="fas fa-users"></i></div>
            <div>
                <p class="text-xs text-slate-500 font-medium">Total Akun Orang Tua</p>
                <p class="text-xl font-bold text-slate-800"><?= count($ortuUsers) ?> <span
                        class="text-sm font-normal text-slate-400">Akun</span></p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            ...
            <div x-show="activeTab === 'admin'" x-transition:enter="transition ease-out duration-300"
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-50 text-sm">
                        ...
                        <tr>
                            <td colspan="4" class="text-center py-6 text-slate-500">Belum ada akun admin.</td>
                        </tr>
                        <?php else : ?>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold">
                            ...
                            <td class="px-6 py-4"><span
                                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-600">Admin</span>
                            </td>
                            <td class="px-6 py-4 text-slate-500"><?= date('d M Y', strtotime($user['created_at'])) ?>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="/admin/akun/edit/<?= $user['id'] ?>"
                                    ...
                                    <a href="/admin/akun/delete/<?= $user['id'] ?>"
            ...
            <div x-show="activeTab === 'guru'" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 transform translate-y-4">
                <table class="w-full text-left">
                    <thead class="bg-slate-50/50 text-slate-600 text-sm border-b border-slate-100">
                        <tr>
                    ...
                    <tbody class="divide-y divide-slate-50 text-sm">
                        ...
                        <tr>
                            <td colspan="4" class="text-center py-6 text-slate-500">Belum ada akun guru.</td>
                        </tr>
                        <?php else : ?>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-9 h-9 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold">
                                        G</div>
                                    <div>
                                        <p class="font-bold text-slate-800"><?= esc($user['username']) ?></p>
                                        <p class="text-xs text-slate-400">ID Akun: #<?= $user['id'] ?></p>
            ...
            <div x-show="activeTab === 'orangtua'" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 transform translate-y-4">
                <table class="w-full text-left">
                    <thead class="bg-slate-50/50 text-slate-600 text-sm border-b border-slate-100">
                        <tr>
                    ...
                    <tbody class="divide-y divide-slate-50 text-sm">
                        ...
                        <tr>
                            <td colspan="4" class="text-center py-6 text-slate-500">Belum ada akun orang tua.</td>
                        </tr>
                        <?php else : ?>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-9 h-9 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center font-bold">
                                        O</div>
                                    <div>
                                        <p class="font-bold text-slate-800"><?= esc($user['username']) ?></p>
                                        <p class="text-xs text-slate-400">ID Akun: #<?= $user['id'] ?></p>
