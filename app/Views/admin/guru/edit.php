            ...
            <div class="md:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-5">
                <div class="flex items-center justify-between border-b border-gray-100 pb-3">
                    ...
                    <div>
                        ...
                        <select name="status"
                            class="mt-0.5 text-xs font-bold border-0 bg-gray-50 rounded-lg focus:ring-0 cursor-pointer">
                            <option value="aktif" <?= old('status', $guru['status']) == 'aktif' ? 'selected' : '' ?>>🟢
                                Aktif</option>
                            <option value="non-aktif"
                                <?= old('status', $guru['status']) == 'non-aktif' ? 'selected' : '' ?>>🔴 Non-Aktif
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700">NIP <span
                                class="text-rose-500">*</span></label>
                        <input type="text" name="nip" value="<?= old('nip', $guru['nip']) ?>" required
                            class="mt-1.5 w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm">
                    </div>
                    <div>
                    ...
                    <div class="mt-2.5 flex space-x-6">
                        <label class="flex items-center text-sm cursor-pointer group">
                            <input type="radio" name="jenis_kelamin" value="L"
                                ...
                                class="text-indigo-600 focus:ring-indigo-500 mr-2 border-gray-300">
                ...
                <div>
                    ...
                    <textarea name="alamat" rows="3"
                        class="mt-1.5 w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm"><?= old('alamat', $guru['alamat']) ?></textarea>
                </div>
            </div>
                    ...
                    <div class="flex items-center space-x-4 mb-2">
                        <div class="shrink-0">
                            ...
                            <div id="default-avatar"
                                class="hidden w-16 h-16 rounded-xl bg-linear-to-br from-indigo-50 to-indigo-100 items-center justify-center text-indigo-600 font-bold text-xl shadow-sm border border-indigo-50">
                                G
                            </div>
                            <img id="preview-foto" src="/uploads/guru/<?= $guru['foto'] ?>" alt="Preview"
                                class="w-16 h-16 rounded-xl object-cover shadow-sm ring-1 ring-gray-100"
                                onerror="this.classList.add('hidden'); const avatar=document.getElementById('default-avatar'); if (avatar) { avatar.classList.remove('hidden'); avatar.classList.add('flex'); }">
                            <?php else : ?>
                            <div id="default-avatar"
                                class="w-16 h-16 rounded-xl bg-linear-to-br from-indigo-50 to-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-xl shadow-sm border border-indigo-50">
                                G
                            </div>
                            <img id="preview-foto"
                                class="hidden w-16 h-16 rounded-xl object-cover shadow-sm ring-1 ring-gray-100" src="#"
                                alt="Preview">
                            <?php endif; ?>
                        </div>

                        <div class="flex-1">
                            <input type="file" name="foto" accept="image/*" onchange="bacaGambar(this)"
                                class="text-xs text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 file:cursor-pointer transition-colors">
