...
<div class="container mx-auto p-6 max-w-7xl">
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        ...
        <a href="/admin/guru/create" class="bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-medium px-5 py-2.5 rounded-xl text-sm flex items-center shadow-lg shadow-indigo-100 transition-all duration-200 transform hover:-translate-y-0.5">
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6">
        <form action="/admin/guru" method="get" class="flex flex-col sm:flex-row gap-3">
            <div class="relative w-full md:w-1/3">
                ...
                <input type="text" name="q" value="<?= esc($keyword) ?>" placeholder="Cari nama atau NIP..." class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm">
            </div>
            <div class="flex gap-2">
                ...
                <?php if ($keyword) : ?>
                    <a href="/admin/guru" class="bg-gray-100 text-gray-700 px-5 py-2.5 rounded-lg hover:bg-gray-200 transition-all text-sm font-medium flex items-center">Reset</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
                            ...
                            <tr class="hover:bg-indigo-50/30 transition-all duration-150">
                                <td class="px-6 py-4">
                                    <div class="flex-shrink-0 w-11 h-11">
                                        ...
                                        <?php else : ?>
                                            <div class="w-full h-full rounded-xl bg-gradient-to-br from-indigo-50 to-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-base shadow-sm border border-indigo-50">
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-indigo-600 text-xs font-bold bg-indigo-50 px-2 py-1 rounded-md border border-indigo-100">
                                        <?= esc($g['nip']) ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-gray-700 font-medium bg-gray-50 px-2.5 py-1 rounded-md text-xs border border-gray-100"><?= esc($g['pendidikan'] ?: 'Belum Diisi') ?></span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-2.5 py-1 text-xs font-bold rounded-full inline-block <?= $g['status'] == 'aktif' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-rose-50 text-rose-700 border border-rose-100' ?>">
                                        <?= ucfirst((string)esc($g['status'])) ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="/admin/guru/detail/<?= $g['id'] ?>" class="text-gray-600 hover:text-emerald-600 font-medium transition-all p-1.5 hover:bg-emerald-50 rounded-lg" title="Detail Profil">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                        </a>
                                        
                                        <a href="/admin/guru/edit/<?= $g['id'] ?>" class="text-gray-600 hover:text-indigo-600 font-medium transition-all p-1.5 hover:bg-indigo-50 rounded-lg" title="Edit">
