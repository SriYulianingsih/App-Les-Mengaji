    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8">
        <form action="/admin/kategori-pembayaran/store" method="POST" class="space-y-6 text-left">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-[11px] font-black uppercase tracking-widest text-slate-500 mb-2">Nama
                        Kategori</label>
                    <input type="text" name="nama_kategori" value="<?= esc(old('nama_kategori') ?? '') ?>"
                        placeholder="Contoh: SPP Bulanan" required
                ...
                <div>
                    <label class="block text-[11px] font-black uppercase tracking-widest text-slate-500 mb-2">Nominal
                        Standar (Rp)</label>
                    <div class="relative">
                        <span
                            class="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-black text-slate-400">Rp</span>
                        <input type="text" id="rupiah" name="nominal_std" value="<?= esc(old('nominal_std') ?? '') ?>"
            ...
            <div class="flex items-center justify-between pt-6 border-t border-slate-50">
                <a href="/admin/kategori-pembayaran"
                    class="text-sm font-bold text-slate-400 hover:text-slate-600">Batal</a>
                <button type="submit"
                    class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 py-3.5 rounded-2xl text-sm shadow-lg shadow-indigo-100 transition-all transform hover:-translate-y-1">
                    Simpan Data
                </button>
            </div>
        </form>
