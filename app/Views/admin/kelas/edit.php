    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <form action="/admin/kelas/update/<?= $kelas['id'] ?>" method="POST" class="p-6 md:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pb-4">
                <div>
                    <label for="nama_kelas" class="block text-[11px] font-black uppercase tracking-widest text-slate-500 mb-2 text-left">Nama Kelas / Kelompok <span class="text-rose-500">*</span></label>
                ...
                <div>
                    <label for="tingkat" class="block text-[11px] font-black uppercase tracking-widest text-slate-500 mb-2 text-left">Jenjang Pendidikan <span class="text-rose-500">*</span></label>
                    <div class="relative group text-left">
                        ...
                        <select name="tingkat" id="tingkat" required
                            ...
                            <optgroup label="Dasar & Tahsin">
                                <option value="Iqra" <?= $currentTingkat == 'Iqra' ? 'selected' : '' ?>>Program Iqra'</option>
                                <option value="Tahsin" <?= $currentTingkat == 'Tahsin' ? 'selected' : '' ?>>Tahsin Tilawah</option>
                            </optgroup>
                            <optgroup label="Madrasah Diniyyah">
                                <option value="Awaliyah" <?= $currentTingkat == 'Awaliyah' ? 'selected' : '' ?>>Diniyyah Awaliyah</option>
                                <option value="Wustha" <?= $currentTingkat == 'Wustha' ? 'selected' : '' ?>>Diniyyah Wustha</option>
                                <option value="Ulya" <?= $currentTingkat == 'Ulya' ? 'selected' : '' ?>>Diniyyah Ulya</option>
                            </optgroup>
                            <optgroup label="Tahfidz & Kitab">
                                <option value="Tahfidz" <?= $currentTingkat == 'Tahfidz' ? 'selected' : '' ?>>Tahfidz Al-Qur'an</option>
                                <option value="Kitab" <?= $currentTingkat == 'Kitab' ? 'selected' : '' ?>>Kajian Kitab Kuning</option>
                            </optgroup>
                        </select>
